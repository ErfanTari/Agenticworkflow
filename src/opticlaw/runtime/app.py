...
from opticlaw.services.llm import build_llm_client
...
class OptiClawApp:
    ...
    def __init__(self, memory_db_path: str = "opticlaw_memory.db") -> None:
        policy_gate = PolicyGate()
        self.event_bus = EventBus()
        self.router = EventRouter(policy_gate)
        self.triage = TriageEngine()
        self.memory = MemoryService(db_path=memory_db_path)
        self.planner = PlannerEngine(ModelRouter(), build_llm_client())
    ...
    async def handle_event(self, event: EventEnvelope) -> None:
        ...
        except Exception as exc:  # resilient runtime boundary
            error_reply = f"OptiClaw encountered an error: {exc}"
            self.responses.append(error_reply)
            await self.memory.store(MemoryRecord(MemoryType.EPISODIC, f"error:{error_reply}", salience=0.9))
